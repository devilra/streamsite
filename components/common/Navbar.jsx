...
export default function Navbar() {
  ...
  const navItems = [
    ...
    { 
      ...
      hasDropdown: true,
      dropdownItems: [
        { title: "Trekking & Hiking", desc: "Conquer the scenic peaks of Yelagiri", icon: Compass },
        { title: "Stargazing Nights", desc: "View clear skies with telescope setups", icon: Sun },
        { title: "Campfire Gatherings", desc: "Stories and acoustic vibes under the stars", icon: Tent },
        { title: "Off-Road Safari", desc: "Thrill-filled forest trail expeditions", icon: Compass },
      ]
    },
    { 
      name: "Stay", 
      ...
      hasDropdown: true,
      dropdownItems: [
        { title: "Luxury Domes", desc: "Climatic controlled glass dome stays", icon: Tent },
        { title: "Mountain Villas", desc: "Spacious private infinity pool villas", icon: Tent },
        { title: "A-Frame Cabins", desc: "Cozy timber chalets with sunset views", icon: Tent },
        { title: "Glamping Tents", desc: "Close-to-nature premium camping", icon: Tent },
      ]
    },
    { name: "Communities", href: "#", hasDropdown: false },
    ...
    <header className="fixed top-4 left-0 right-0 z-50 px-4 md:px-8 max-w-7xl mx-auto">
      ...
      <nav className="relative flex items-center justify-between px-4 py-2.5 rounded-2xl border border-white/10 dark:border-white/10 light:border-black/5 bg-black/40 dark:bg-black/40 light:bg-white/80 backdrop-blur-xl shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] transition-all duration-300">
        ...
        <a href="#" className="flex items-center gap-3 group select-none">
          ...
          <div className="relative w-10 h-10 flex items-center justify-center text-white dark:text-white light:text-slate-900">
            ...
            >
              {/* Stars */}
              {/* Center Main Star */}
              <path d="M50,12 L52,18 L58,20 L52,22 L50,28 L48,22 L42,20 L48,18 Z" fill="currentColor" className="text-lime-400 stroke-none" />
              {/* Left Star */}
              <path d="M28,24 L29,27 L32,28 L29,29 L28,32 L27,29 L24,28 L27,27 Z" fill="currentColor" className="text-white/80 stroke-none" />
              {/* Right Star */}
              <path d="M72,24 L73,27 L76,28 L73,29 L72,32 L71,29 L68,28 L71,27 Z" fill="currentColor" className="text-white/80 stroke-none" />
              ...
              <path d="M30,87 H70" strokeWidth="1.5" className="opacity-30" strokeDasharray="6 4" />
          ...
          {navItems.map((item) => (
            ...
            >
              {item.hasDropdown ? (
                <div>
                  <button
                    onClick={() => setActiveDropdown(activeDropdown === item.name.toLowerCase() ? null : item.name.toLowerCase())}
                    className={cn(
                      ...
                      activeItem === item.name || activeDropdown === item.name.toLowerCase()
                        ? "text-lime-400" 
                  ...
                  >
                    ...
                    <ChevronDown 
                      className={cn(
                        ...
                        activeDropdown === item.name.toLowerCase() && "rotate-180 text-lime-400"
                      )} 
                    />
                  </button>
                ...
                <button
                  ...
                  className={cn(
                    "relative px-4 py-2 text-sm font-medium rounded-lg transition-colors hover:text-white dark:hover:text-white light:hover:text-black",
        ...
        <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              ...
              <AnimatePresence>
                {activeDropdown === "theme" && (
                  ...
                  >
                    {[
                      { id: "light", label: "Light", icon: Sun, color: "text-lime-500 dark:text-lime-400" },
                      { id: "dark", label: "Dark", icon: Moon, color: "text-indigo-500 dark:text-indigo-400" },
                      { id: "system", label: "System", icon: Monitor, color: "text-sky-500 dark:text-sky-400" },
                    ].map((opt) => {
                      const Icon = opt.icon;
          ...
          <button className="relative hidden md:flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-black bg-lime-400 hover:bg-lime-300 transition-all duration-300 shadow-[0_0_20px_rgba(163,230,53,0.3)] hover:shadow-[0_0_25px_rgba(163,230,53,0.55)] active:scale-95 group/btn overflow-hidden">
            ...
            >
              <div>
                ...
                <div className="mt-8 flex flex-col gap-1.5">
                  {navItems.map((item) => {
                    const isDropdownActive = activeDropdown === item.name.toLowerCase()
                    return (
                      <div key={item.name} className="flex flex-col">
                        {item.hasDropdown ? (
                          <>
                            <button
                              onClick={() => setActiveDropdown(isDropdownActive ? null : item.name.toLowerCase())}
                              className={cn(
                                ...
                                isDropdownActive && "text-lime-400 bg-white/5"
                              )}
                            >
                              <span>{item.name}</span>
                              <ChevronDown className={cn("w-4 h-4 transition-transform duration-300", isDropdownActive && "rotate-180 text-lime-400")} />
                            </button>
                                  ...
                                  <div className="p-2 grid gap-1">
                                    {item.dropdownItems.map((subItem) => (
                                      ...
                                      >
                                        ...
                                        <div className="text-[10px] opacity-60 mt-0.5">{subItem.desc}</div>
                                      </a>
                                    ))}
                                  </div>
                                </motion.div>
              ...
              <div className="pt-6 border-t border-white/10 dark:border-white/10 light:border-black/10 flex flex-col gap-4">
                <button className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-sm font-bold text-black bg-lime-400 hover:bg-lime-300 transition-colors shadow-[0_0_15px_rgba(163,230,53,0.3)]">
                  <Calendar className="w-4 h-4 stroke-[2.5]" />
                  <span>Book Now</span>
                </button>
              </div>
